...
def fetch_price_data(ticker, start_date="2021-06-01", end_date="2025-12-31"):
    ...
    try:
        ...
        print(f"  [WARN] Unable to fetch {ticker} price data: {e}")
        return None


# ============================================================
# Part 2: Manually entered key quarterly fundamentals
# (Quarterly financials from the API are unreliable; manually entering the core data is more accurate)
# ============================================================
...
FUNDAMENTALS = {
    "NVDA": {
        ...
        "quarters": OrderedDict([
            # (earnings release date, {revenue in $100M, revenue YoY growth, gross margin, EPS, EPS beat %})
            ...
            # FY2024 = calendar 2023 — AI explosion
            ("2023-05-24", {"rev": 71.9, "rev_yoy": -13.0, "gm": 64.6, "eps": 1.09, "eps_beat": 18.5, "label": "FY24Q1 (Apr23) ★ AI inflection"}),
            ("2023-08-23", {"rev": 135.1, "rev_yoy": 101.0, "gm": 70.1, "eps": 2.70, "eps_beat": 29.0, "label": "FY24Q2 (Jul23) ★★ explosion"}),
...
# ============================================================
# Part 3: Momentum discovery engine (first filter layer)
# ============================================================
...
# ============================================================
# Part 4: Value verification engine (second filter layer)
# ============================================================
...
def verify_value(ticker, fund_data, prev_fund_data=None):
    ...
    # 1. Revenue acceleration (is revenue YoY growth improving)
    ...
    checks["Revenue acceleration"] = rev_accelerating

    # 2. Gross margin direction (>45% and not shrinking)
    ...
    if prev_fund_data:
        ...
        gm_expanding = gm > 45
    checks["Gross margin expansion"] = gm_expanding

    # 3. EPS beat (>10% is a strong signal)
    eps_beat = fund_data.get("eps_beat", 0)
    checks["Earnings surprise"] = eps_beat > 10

    # 4. Revenue growth itself (>15%)
    checks["High revenue growth"] = rev_yoy > 15

    # 5. Absolute gross margin (>40%, chip-industry standard)
    checks["Healthy gross margin"] = gm > 40
...
def backtest_ticker(ticker):
    ...
    if not prices:
        print("  ❌ Unable to fetch price data, skipping")
        return None

    print(f"  Fetched {len(prices)} trading days of data ({prices[0]['date']} ~ {prices[-1]['date']})")
    ...
    # Show only signals within the key time window
    first_buy = None
    for bs in buy_signals:
        if bs["date"] >= "2022-06-01":
            ...
            print(f"     Momentum: 30-day gain {bs['pct_30d']}% | volume multiple {bs['vol_ratio']}x")
    ...
    if first_buy and prices:
        ...
        # Find the price 1 year and 2 years later
        for p in prices:
            if p["date"] >= buy_date:
                final_price = p["close"]
...
if __name__ == "__main__":
    ...
    print(f"{'='*70}")
    print(f"\n  {'Target':<8} {'First buy signal':<16} {'Buy price':<12} {'Triggering fundamentals'}")
    print(f"  {'—'*65}")
    ...
    print(f"  │ See the detailed analysis above for the answer.           │")
    print(f"  └─────────────────────────────────────────────────────────────┘")
