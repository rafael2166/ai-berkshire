...
NVDA: key points entered manually (Yahoo API is throttled)
AMD/MU: real daily data loaded from JSON files
"""
...
# ============================================================
# Fundamental data (entered manually, more accurate than the API)
# ============================================================
...
FUNDAMENTALS = {
    "NVDA": {
        ...
        "quarters": OrderedDict([
            ...
            ("2023-08-23", {"rev": 135.1, "rev_yoy": 101.0, "gm": 70.1, "eps_beat": 29.0, "label": "FY24Q2(Jul23) ★★explosion! revenue doubled"}),
    ...
    "AMD": {
        ...
        "quarters": OrderedDict([
            ...
            ("2023-10-31", {"rev": 58.0, "rev_yoy": 4.0, "gm": 47.0, "eps_beat": 6.1, "label": "Q3 2023 rebound begins"}),
...
def verify(fund, prev_fund):
    ...
    if pd:
        checks["Revenue acceleration"] = d["rev_yoy"] > pd["rev_yoy"]
    else:
        checks["Revenue acceleration"] = d["rev_yoy"] > 20

    # 2. Gross margin direction
    if pd:
        checks["Gross margin↑"] = d["gm"] > pd["gm"] or d["gm"] > 50
    else:
        checks["Gross margin↑"] = d["gm"] > 40

    # 3. EPS beat > 10%
    checks["Earnings surprise"] = d["eps_beat"] > 10

    # 4. High revenue growth > 15%
    checks["High revenue growth"] = d["rev_yoy"] > 15

    # 5. Gross margin > 40%
    checks["Healthy gross margin"] = d["gm"] > 40
...
def backtest(ticker, prices):
    ...
    # Show some rejected signals (helps understand the filtering effect)
    ...
    if first_buy:
        ...
        print(f"\n  {'='*60}")
        print(f"  📊 Return from the first buy signal:")
        print(f"     Buy: {first_buy['date']} @ ${first_buy['close']}")
        print(f"     Held to: {final['date']} @ ${round(final['close'], 2)}")
...
def nvda_manual_analysis():
    ...
    print(f"  (Yahoo API throttled, using known historical price points)")
    print(f"{'='*70}")

    # NVDA key price points (split-adjusted)
    key_prices = [
        ("2022-10-14", 11.2, "intraday low of the year"),
        ("2023-01-06", 14.3, "first wave after the ChatGPT catalyst"),
        ("2023-01-27", 19.9, "★ new 60-day high + volume breakout → momentum triggered"),
        ("2023-02-22", 23.4, "FY23Q4 earnings: gross margin 63.3% inflection + EPS beat 10%"),
        ("2023-05-24", 30.5, "before FY24Q1 earnings"),
        ("2023-05-25", 37.9, "★★ FY24Q1 gap up 24% post-earnings: revenue beat 18.5%"),
        ("2023-08-24", 49.3, "FY24Q2: revenue doubled 101%"),
        ("2024-01-08", 52.2, "CES 2024"),
        ("2024-03-08", 87.5, "approaching all-time high"),
    ...
    print(f"     Price signal: rose from $11.2 to $19.9 (+78%/3 months), new 60-day high + clear volume surge")
    print(f"     Fundamentals at the time (FY23Q3 Oct22): revenue YoY -17% | gross margin 53.6% | EPS beat 7.4%")
    ...
    if s1 >= 3:
        ...
        print(f"     Judgment: ❌ Rejected (revenue still declining, but gross margin has turned up)")
        print(f"     Note: this is a borderline signal — the framework did not give a buy, but the 63.3% gross margin inflection is a real signal")
    ...
    print(f"\n  📅 2023-05-25  $37.9  ★★FY24Q1 'AI bombshell' earnings")
    ...
    print(f"     Judgment: ✅ Perfect-score signal! 5/5 all pass")

    # Return calculation
    scenarios = [
        ("2023-01-27 (borderline signal)", 19.9, 149.4, "2025-01"),
        ("2023-02-22 (earnings confirmation)", 23.4, 149.4, "2025-01"),
        ("2023-05-25 (AI bombshell)", 37.9, 149.4, "2025-01"),
    ]
    print(f"\n  {'='*60}")
    print(f"  📊 Returns from different buy points (held to 2025-01 $149.4):")
    print(f"  {'—'*60}")
...
if __name__ == "__main__":
  ...
  │  - Most certain signal: 2023-05-25 after FY24Q1 "AI bombshell" earnings │
  │  - The framework fires at the ChatGPT catalyst + gross margin inflection │
  │  - Even confirming a buy at the latest 2023-05, holding to 2025 still gives +294% │
  │                                                                │
  │  AMD: see the actual backtest results ↑                       │
  │  - Expected: triggers 2023-10 ~ 2024-01 (MI300 launch + revenue rebound) │
  │                                                                │
  │  MU: see the actual backtest results ↑                        │
  │  - Expected: triggers 2023-12 ~ 2024-03 (HBM demand + revenue reversal + big EPS beat) │
  └────────────────────────────────────────────────────────────────┘

  Core conclusions:
  1. The framework is most effective on NVDA — "gross margin inflection + EPS beat" is the strongest early signal
  2. A pure value investor would miss the early-2023 entry because "revenue is still declining"
  3. A pure momentum investor would chase NVDA high in 2022 and take losses
  4. Advantage of the "momentum + value" combo: enter only after a price breakout + fundamental confirmation
     avoiding the false breakout of 2022 and catching the real inflection of 2023

  Limitations of the framework:
  1. If "revenue YoY >15%" is strictly required, it would miss NVDA's first signal in 2023-01
     → suggest adding "gross margin improving consecutively" as an independent buy condition
  2. For cyclical stocks (MU) adjustment is needed: a big revenue drop at the semiconductor cycle bottom is normal
     → suggest adding "EPS beat magnitude >30%" as a special condition for cyclical stocks
""")
